...
import { useRegisterStealfWallet } from '@/src/features/umbra/hooks/useRegisterStealfWallet';
import { usePrefetchWalletData } from '@/src/features/bank/hooks/usePrefetchWalletData';
import { useSetupStealfWallet } from '@/src/features/umbra/hooks/useSetupStealfWallet';
import type { SetupChoice } from '@/src/features/umbra/screens/StealfWalletSetup';
import {
  decideSyncAction,
  runSyncScan,
} from '@/src/features/umbra/lib/syncStealfWallet';
...
/**
 * Owns the create/import → persist → claim-scan flow for the stealth wallet.
 * Shared by the Payment-tab gate (`StealthWalletGate`) and the Home takeover
 * so the orchestration lives in exactly one place.
 */
export function useStealfWalletSetupFlow() {
  ...
  const persistStealfWallet = async (
    ...
    try {
      ...
      if (__DEV__) console.warn('[useStealfWalletSetupFlow] register failed:', msg);
    ...
    try {
      ...
      if (decision.action === 'scan') {
        ...
        try {
          ...
          if (__DEV__)
            console.warn('[useStealfWalletSetupFlow] sync scan failed (continuing):', err);
        } finally {
          setSyncProgress(null);
        }
      }
    } catch (err) {
      if (__DEV__) console.warn('[useStealfWalletSetupFlow] sync decision threw:', err);
    }
  };
