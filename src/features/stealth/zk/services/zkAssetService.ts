...
async function validateManifestVersion(): Promise<void> {
  ...
  try {
    const local = JSON.parse(await mFile.text()) as Partial<LocalZkManifest>;
    ...
    if (local.manifestVersion !== remote.version) {
      if (__DEV__) {
        console.log(
          `[ZK] Manifest version mismatch (local: ${local.manifestVersion}, remote: ${remote.version}) — clearing.`,
        );
      }
      await clearZkAssetsCache();
    }
...
async function getCachedUri(
  ...
  if (await isRemoteVersionNewer(type, variant, localAsset.version)) {
    return null;
  }
  return file.uri;
}
